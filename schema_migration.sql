...
-- Updates your current schema to support the enhanced achievement system
...
-- Add missing column to achievement_progress table
...
-- Update existing user_achievements with default category based on achievement_id
UPDATE public.user_achievements SET 
  achievement_category = CASE 
    ...
    ELSE 'special' -- Default for any unrecognized achievements
  END
WHERE achievement_category IS NULL;

-- Update existing user_achievements with titles and descriptions
UPDATE public.user_achievements SET 
  achievement_title = CASE achievement_id
    ...
    ELSE achievement_id -- Fallback to ID if not found
  END,
  ...
  icon_name = CASE achievement_id
    ...
    WHEN 'billionaire' THEN '57669' -- Icons.auto_awesome
    -- Streak achievements
    WHEN 'winning_streak' THEN '58614' -- Icons.local_fire_department
    WHEN 'marathon_trader' THEN '983325' -- Icons.run_circle
    WHEN 'perfectionist' THEN '983427' -- Icons.star_rate
    -- Special achievements
    WHEN 'diamond_hands' THEN '57673' -- Icons.diamond_outlined
    WHEN 'paper_hands' THEN '983204' -- Icons.flash_on
    WHEN 'early_bird' THEN '983991' -- Icons.wb_sunny
    WHEN 'night_owl' THEN '59159' -- Icons.nightlight_round
    WHEN 'weekend_warrior' THEN '983991' -- Icons.weekend
    WHEN 'high_roller' THEN '57427' -- Icons.casino
    WHEN 'penny_pincher' THEN '983427' -- Icons.savings
    WHEN 'day_trader' THEN '57404' -- Icons.flash_auto
    WHEN 'swing_trader' THEN '57404' -- Icons.trending_neutral
    WHEN 'value_investor' THEN '57404' -- Icons.schedule
    ELSE '57404'
  END,
  color_hex = CASE achievement_id
    ...
    ELSE '#3498db'
  END
WHERE achievement_title IS NULL OR achievement_description IS NULL OR icon_name IS NULL OR color_hex IS NULL;
...
-- ============================================================================
-- 5. CLEAN UP OLD SECTOR-BASED ACHIEVEMENTS (Optional)
-- ============================================================================
...
-- SELECT COUNT(*) as total_achievements FROM public.user_achievements;
-- SELECT achievement_category, COUNT(*) as count FROM public.user_achievements GROUP BY achievement_category;
-- SELECT COUNT(*) as achievements_with_metadata FROM public.user_achievements WHERE achievement_title IS NOT NULL;
...
-- Next steps:
-- 1. Run this schema_migration.sql script
-- 2. Then run comprehensive_database_fixes.sql
-- 3. Optionally run the verification queries above